...
class Game:
	...
	def get_player_list(self):
		...
		for player in self.player_list:
			if i not in self.players_out_of_game:
				liste.append(player)
			i += 1
		return liste
	...
	def set_next_player(self):
		if self.current_die_number != 6:
			...
			while self.current_player in self.players_out_of_game and len(self.players_out_of_game) != len(self.player_list):
				self.current_player += 1
				if self.current_player >= self.number_of_players:
					self.current_player -= self.number_of_players
	...
	def move(self, field):
		...
		if figures < 4 and self.current_die_number == 6:
			if self.current_player * 10 not in self.field or self.current_player * 10 in self.field and self.field[self.current_player * 10] != self.current_player:
				can_move.append(-1)
		...
		if field in self.field:
			...
			for x in self.field.keys():
				...
				if self.field[x] == self.current_player:
					...
					if new_field >= self.current_player * 10 and x < self.current_player * 10:
						continue
					if new_field in self.field and self.field[new_field] != self.field[x]:
						print "X: ", x
						has_to_kick.append(x)
			...
			if new_field in self.field.keys():
				if self.field[new_field] == self.current_player:
					return False
			
			# move!
			del self.field[field]
			self.field[new_field] = self.current_player
			if not field in has_to_kick:
				for x in range(len(has_to_kick)):
					del self.field[has_to_kick[0]]
					del has_to_kick[0]
